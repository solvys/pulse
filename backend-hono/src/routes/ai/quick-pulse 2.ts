...
export const quickPulseRoute = new Hono<{ Variables: Variables }>();
...
quickPulseRoute.post('/', async (c) => {
    try {
      ...
      LIMIT 10
    `;
        const newsContext = newsRows.map((n: { source: string; title: string; sentiment: string }) => `- [${n.source}] ${n.title} (${n.sentiment})`).join('\n');
