...
const FEATURE_TIER_MAP: Record<string, BillingTier> = {
...
const TIER_HIERARCHY: Record<BillingTier, number> = {
  'free': 0,
  'pulse': 1,
  'pulse_plus': 2,
  'pulse_pro': 3,
 ...
 */
export async function getUserBillingTier(userId: string): Promise<BillingTier | null> {
  try {
...
export function checkFeatureAccess(
  ...
  featureName: string
): { hasAccess: boolean; requiredTier: BillingTier } {
 ...
 */
export async function setUserBillingTier(
  userId: string,
  tier: BillingTier
): Promise<boolean> {
  try {
