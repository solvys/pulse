...
export const INSTRUMENT_BETAS: Record<string, { beta: number; notes: string }> = {
  '/ES': { beta: 1.0, notes: 'Base - S&P 500' },
  '/NQ': { beta: 1.2, notes: 'Tech-heavy - Nasdaq 100' },
  '/MNQ': { beta: 1.2, notes: 'Micro Nasdaq' },
  '/YM': { beta: 0.95, notes: 'Industrials - Dow' },
  '/RTY': { beta: 1.1, notes: 'Small caps - Russell' },
  '/GC': { beta: 0.2, notes: 'Safe-haven, low correlation' },
  '/SI': { beta: 0.4, notes: 'Industrial/vol proxy' },
}
...
export function calculateImpliedPoints(
  ...
  // Rule of 16: Implied % = VIX / 16
  const impliedPct = vixLevel / 16
  
  // Base points for the instrument
  const basePoints = currentPrice * (impliedPct / 100)
  
  // Get beta for instrument
  const instrumentConfig = INSTRUMENT_BETAS[instrument] ?? { beta: 1.0, notes: 'Unknown' }
  
  // Adjusted points = Base * Beta
  const adjustedPoints = basePoints * instrumentConfig.beta
