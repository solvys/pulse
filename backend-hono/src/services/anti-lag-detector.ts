...
function getAssetClass(symbol: string): 'risk' | 'safe_haven' | 'unknown' {
 ...
 */
export async function detectAntiLag(
  userId: string,
  primarySymbol: string,
  correlatedSymbol: string,
  lookbackSeconds: number = 60
): Promise<AntiLagDetectionResult> {
  ...
  let eventType: 'anti_lag' | 'contra_anti_lag' = 'anti_lag';
 ...
 */
export async function recordAntiLagEvent(
  userId: string,
  event: AntiLagEvent
): Promise<void> {
  await sql`
 ...
 */
export function getAvailableCorrelatedPairs(
  symbol: string
): Array<{ symbol: string; assetClass: 'risk' | 'safe_haven' | 'unknown'; recommended: boolean }> {
  ...
  if (assetClass === 'safe_haven') {
    ...
    const safeHavens = SAFE_HAVENS
      ...
      .map(asset => ({
        symbol: asset,
        assetClass: 'safe_haven' as const,
        recommended: true,
      }));
 ...
 */
export function checkAssetClassMatch(
  primarySymbol: string,
  correlatedSymbol: string
