...
export async function getThreatHistory(
  ...
  const formatted: Threat[] = threats.map((row: any) => ({
    id: row.id.toString(),
    type: row.type as 'overtrading' | 'emotional' | 'consecutive_losses',
    severity: row.severity as 'low' | 'medium' | 'high' | 'critical',
    description: row.description || '',
    timestamp: row.createdAt.toISOString(),
    metadata: (row.metadata as ThreatMetadata) || {},
  }));
...
export async function detectOvertrading(
  ...
  durationWindow: string = '24 hours'
): Promise<Threat | null> {
  ...
  if (currentCount > usualTradesPerDuration) {
    ...
    const severity =
      excess > usualTradesPerDuration * 0.5
        ? 'critical'
    ...
    return {
      ...
      metadata: (threat.metadata as ThreatMetadata) || {},
    };
  }
 ...
 */
export async function detectEmotionalTrading(
  userId: string
): Promise<Threat | null> {
  ...
  if (emotionalIndicators.length > 0 || triggeredBlindSpots.length > 0) {
    const severity =
      emotionalIndicators.length >= 2 || triggeredBlindSpots.length >= 3
        ? 'high'
        : 'medium';
 ...
 */
export async function getDailyPnL(userId: string, date?: string): Promise<number> {
...
export async function detectThreats(
  ...
  if (usualTradesPerDuration) {
    const overtrading = await detectOvertrading(
      userId,
      usualTradesPerDuration,
      durationWindow
    );
    if (overtrading) {
...
export async function logThreat(
  ...
  return {
    id: threat.id.toString(),
    type: threat.type as 'overtrading' | 'emotional' | 'consecutive_losses',
    severity: threat.severity as 'low' | 'medium' | 'high' | 'critical',
    description: threat.description || '',
    timestamp: threat.createdAt.toISOString(),
    metadata: (threat.metadata as ThreatMetadata) || {},
