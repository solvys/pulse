 ...
 */
function isGuardRailed(name) {
    return GUARD_RAILED_BLIND_SPOTS.some((guarded) => guarded.toLowerCase() === name.toLowerCase());
}
/**
 * Get all blind spots for a user
 * Returns data formatted for 3-column checkbox dropdown
 */
export async function getUserBlindSpots(userId) {
  ...
  `;
    const formatted = blindSpots.map((row) => ({
        id: row.id.toString(),
        name: row.name,
 ...
 */
export async function getActiveBlindSpots(userId) {
    const blindSpots = await sql `
    SELECT 
      id, user_id as "userId", name, is_guard_railed as "isGuardRailed",
      is_active as "isActive", category, source, created_at as "createdAt"
 ...
 */
export async function upsertBlindSpot(userId, blindSpotId, data) {
 ...
 */
export async function deleteBlindSpot(userId, blindSpotId) {
 ...
 */
export async function createAIDeterminedBlindSpot(userId, name, category = 'behavioral') {
 ...
 */
export async function toggleBlindSpot(userId, blindSpotId, isActive) {
    const [updated] = await sql `
    UPDATE blind_spots
...
//# sourceMappingURL=blind-spots-service.js.map