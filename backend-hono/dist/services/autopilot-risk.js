 ...
 */
async function apiCall(url, options, cacheKey, cacheTTL) {
    // Check cache
 ...
 */
export async function checkThreatHistory(userId) {
    try {
 ...
 */
export async function checkBlindSpots(userId) {
    try {
        // TODO: Replace with real endpoint when Agent 1 is ready
        ...
        const guardRailedActive = result.blindSpots.filter((bs) => bs.isGuardRailed === true && bs.isActive === true);
        const riskCategoryActive = result.blindSpots.filter((bs) => bs.category === 'risk' && bs.isActive === true && bs.isGuardRailed === false);
 ...
 */
export async function getIVScore(userId, symbol) {
    try {
        // TODO: Replace with real endpoint when Agent 1 is ready
 ...
 */
export async function validateTradingFrequency(userId, accountId) {
    try {
 ...
 */
export async function validateRisk(userId, accountId, proposalSize, proposalPrice) {
    const reasons = [];
    try {
        // Get account balance
...
//# sourceMappingURL=autopilot-risk.js.map