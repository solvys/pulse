...
export async function createStreamingChatResponse(uiMessages, modelName = 'grok-4', context, onFinish, enableTools = false) {
    ...
    const fallbackModels = (modelName === 'claude-opus-4' || modelName === 'claude-opus-4-20250514')
    ...
    const modelMessages = await convertToModelMessages(messagesForConversion);
    const messages = modelMessages.some((msg) => msg.role === 'system')
        ? modelMessages
        : [{ role: 'system', content: systemPrompt }, ...modelMessages];
    let lastError = null;
...
export async function* streamAIResponse(prompt, conversationHistory, modelName = 'grok-4', context) {
    try {
