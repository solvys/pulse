 ...
 */
async function fetchVIXFromGoogleFinance() {
    try {
 ...
 */
function calculateBaseScore(vix) {
...
function getColorFromScore(score) {
    if (score === 1)
        return 'gray';
    if (score >= 3 && score <= 5.9)
        return 'green';
    if (score >= 6 && score <= 8.4)
        return 'orange';
    return 'red';
}
/**
 ...
 */
function calculateImpliedPoints(ivPercentage, instrumentTickValue = 5.0) {
 ...
 * Cache a score
 */
function cacheScore(score, symbol) {
    const cacheKey = symbol || 'general';
 ...
 */
export async function calculateIVScore(userId, symbol, instrument) {
    // Check cache first
 ...
 */
export async function getCurrentIVScore(userId, symbol) {
    return calculateIVScore(userId, symbol);
}
/**
 * Get IV score history
 */
export async function getIVScoreHistory(userId, symbol, limit = 100, offset = 0) {
    let scores;
    let countResult;
    if (symbol) {
        scores = await sql `
 ...
 */
export function clearScoreCache(symbol) {
    if (symbol) {
