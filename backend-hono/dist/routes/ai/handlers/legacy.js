...
export async function handleGetUserSettings(c) {
    try {
        return c.json({
...
export async function handleCheckTape(c) {
    const userId = c.get('userId');
    try {
        const ivScore = await getCurrentIVScore(userId);
        const vix = await getCurrentVIX();
        const analysis = await generateQuickPulseAnalysis(ivScore.score, vix.value, { timestamp: new Date().toISOString() });
        return c.json({
            message: analysis,
            insights: [
...
export async function handleGenerateDailyRecap(c) {
    const userId = c.get('userId');
    try {
