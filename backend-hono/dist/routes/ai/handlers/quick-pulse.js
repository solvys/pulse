...
export async function handleQuickPulse(c) {
    ...
    try {
        ...
        if (screenshot && typeof screenshot !== 'string') {
            ...
            screenshotAnalysis = await analyzeImage(buffer, 'Analyze this TopStepX trading interface screenshot. Identify current market state, positions, and key indicators.', `Current VIX: ${vix.value}, IV Score: ${ivScore.score}`);
        }
        const analysis = await generateQuickPulseAnalysis(ivScore.score, vix.value, undefined, screenshotAnalysis);
...
export async function handleGetCachedPulse(c) {
    const userId = c.get('userId');
    try {
    ...
    `;
        ...
        const analysisData = cached.analysis;
        return c.json({
            ...analysisData,
