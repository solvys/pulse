...
export async function handleChat(c) {
    ...
    `;
        const newsContext = recentNews.length > 0
            ? [`Recent high-priority news (${recentNews.length} items):\n${recentNews.map((n) => `- [Level ${n.macro_level}] ${n.title} (${n.source}) - ${n.price_brain_sentiment || 'Neutral'} / ${n.price_brain_classification || 'Neutral'}`).join('\n')}`]
            : [];
        const context = [...blindSpotContext, ...newsContext];
        const formattedUIMessages = uiMessages.map((msg) => ({
            id: msg.id || `msg-${Date.now()}-${Math.random()}`,
            role: msg.role,
            content: msg.content || (msg.parts?.find((p) => p.type === 'text')?.text || ''),
            parts: msg.parts || [{ type: 'text', text: msg.content }],
        }));
        const streamingResponse = await createStreamingChatResponse(formattedUIMessages, model, context, async (fullResponse) => {
            try {
                await sql `
            INSERT INTO ai_messages (conversation_id, role, content)
