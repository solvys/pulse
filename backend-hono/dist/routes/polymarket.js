...
import { fetchAllPolymarketOdds, checkSignificantChanges, createPolymarketUpdate, } from '../services/polymarket-service.js';
...
polymarketRoutes.post('/sync', async (c) => {
    try {
      ...
      `;
            ...
            if (previous) {
                const changeCheck = await checkSignificantChanges(odds);
                if (changeCheck.hasChange) {
                    const update = createPolymarketUpdate(odds.marketType, Number(previous.yes_odds), odds.yesOdds);
                    await sql `
